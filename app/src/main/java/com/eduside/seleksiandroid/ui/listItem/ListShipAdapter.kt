...
class ListShipAdapter @Inject constructor() :
    ListAdapter<ShipVo, ListShipAdapter.ViewHolder>(ListDiffUtill()) {
...
//            .into(holder.binding.imgKambing)
        holder.binding.cvContainer.setOnClickListener {
            EventBus.getDefault().post(ItemDataShipEvent(data))
        }
    }
    ...
    class ListDiffUtill : DiffUtil.ItemCallback<ShipVo>() {
        override fun areItemsTheSame(oldItem: ShipVo, newItem: ShipVo): Boolean {
            return newItem.id== oldItem.id
        }

        override fun areContentsTheSame(oldItem: ShipVo, newItem: ShipVo): Boolean {
